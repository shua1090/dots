#!/usr/bin/env python3
"""
Fetch GitHub PR data using gh CLI and organize it for local review.

Usage:
    python fetch_pr_data.py <pr_url> [--output-dir <dir>] [--clone]
...
Example:
    ...
    python fetch_pr_data.py https://github.com/owner/repo/pull/123 --clone
"""
...
def main():
    ...
    parser.add_argument('--clone', action='store_true',
                       help='Clone the PR branch into the review directory. Not used by the default read-only local-checkout workflow.')
    parser.add_argument('--no-clone', action='store_true',
                       help='Deprecated compatibility flag. Cloning is skipped by default.')
    ...
    try:
        ...
        # Clone repository and get diff (optional, disabled by default)
        git_diff = ""
        if args.clone and not args.no_clone:
            try:
